...
- Old ROG-Map runtime files have since been removed from `sim_ws`; archive/history docs remain as reconstruction context.

Consequence:

- Stage 2 now produces `/perception/costmap_2d` directly from `/cloud_registered`, `/Odometry`, a static world prior, and map-frame TF.
- ROG tuning is not available in the current `sim_ws` runtime; reconstruct that path first if a task explicitly needs comparison.
...
- `path_tracker`, `goal_controller`, and `pure_pursuit` are not part of the current Stage 3 controller path and have been removed from the active workspace.
